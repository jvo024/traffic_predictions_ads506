...
autoplot(junc_3.ts, main = "Junction 3") +
  labs(title = "Hourly Vehicles at Junction 3",
       x = "Time", 
       y = "# Vehicles ")

junc_4.ts <- ts(junc_4$Vehicles, frequency = 24)
autoplot(junc_4.ts, main = "Junction 4") +
  labs(title = "Hourly Vehicles at Junction 4",
       x = "Time", 
       y = "# Vehicles ")
```
...
# Junction 3 
j3_11 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(11)], ])$Vehicles)
j3_12 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(12)], ])$Vehicles)
j3_1 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(1)], ])$Vehicles)
j3_2 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(2)], ])$Vehicles)
j3_3 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(3)], ])$Vehicles)
j3_4 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(4)], ])$Vehicles)
j3_5 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(5)], ])$Vehicles)
j3_6 <- sum((junc_3[months(junc_3$DateTime) %in% month.name[c(6)], ])$Vehicles)

j3_vehicle_totals = c(j3_11, j3_12, j3_1, j3_2, j3_3, j3_4, j3_5, j3_6)
j3_months = c('Nov', 'Dec', 'Jan', 'Feb', 'Mar', 'April', 'May', 'June')

# Junction 4 
j4_1 <- sum((junc_4[months(junc_4$DateTime) %in% month.name[c(1)], ])$Vehicles)
j4_2 <- sum((junc_4[months(junc_4$DateTime) %in% month.name[c(2)], ])$Vehicles)
j4_3 <- sum((junc_4[months(junc_4$DateTime) %in% month.name[c(3)], ])$Vehicles)
j4_4 <- sum((junc_4[months(junc_4$DateTime) %in% month.name[c(4)], ])$Vehicles)
j4_5 <- sum((junc_4[months(junc_4$DateTime) %in% month.name[c(5)], ])$Vehicles)
j4_6 <- sum((junc_4[months(junc_4$DateTime) %in% month.name[c(6)], ])$Vehicles)
...
#j3
j3$Date <- as.Date(junc_3$DateTime)
j3<- aggregate(j3["Vehicles"], by=j3["Date"], sum)
j3[ , 'Junction'] <- 3

#j4 
j4$Date <- as.Date(j4$DateTime)
j4<- aggregate(j4["Vehicles"], by=j4["Date"], sum)
j4[ , 'Junction'] <- 4

```
### 3.2.2 Identify Holidays
...
junc_3_date <- as.Date(junc_3$DateTime)
junc_4_date <- as.Date(junc_4$DateTime)

# Create Holiday column by testing if Holiday 
j1[ , 'Holiday'] <- as.Date(j1$Date) %in% bank_holidays
j2[ , 'Holiday'] <- as.Date(j2$Date) %in% bank_holidays
j3[ , 'Holiday'] <- as.Date(j3$Date) %in% bank_holidays
j4[ , 'Holiday'] <- as.Date(j4$Date) %in% bank_holidays

## Initialize aggregated data as time series, weekly seasonality 
j1.ts <- ts(j1$Vehicles, frequency = 7)
j2.ts <- ts(j2$Vehicles, frequency = 7)
j3.ts <- ts(j3$Vehicles, frequency = 7)
j4.ts <- ts(j4$Vehicles, frequency = 7)
```
...
## Junction 2  - Shows multiplicative trend? and seasonality. Possible outlier around season 50
autoplot(j2.ts) +
  labs(title = "Vehicles at Junction 2",
       x = "Time", 
       y = "# Vehicles ")

## Junction 3 - Shows slight upward trend. High variation in the number of vehicles per day.
 # Possible outliers that are that high? Or just peak times 
autoplot(j3.ts)+
  labs(title = "Vehicles at Junction 3",
       x = "Time", 
       y = "# Vehicles ")

## Junction 4 -  No apparent trend, mean seems to be 0. High variation in the number of vehicles per day. 
 # Possible outliers that are that high? Or just peak times 
autoplot(j4.ts) +
...
acf(j3.ts)   # ACF plot shows multiplicative seasonality? Seasons/cyclical nature is not pronounced 
acf(j4.ts)   # ACF plot shows multiplicative seasonality? Seasons are less correlated as time goes on 
...
j1.results <- data.frame(Model = c("Naive", "Simple Exponential Smoothing", "Holt Winters", "Regression"),
                 RMSE = c(j1.naive.RMSE, j1.se.RMSE, j1.holt.RMSE, j1.reg.RMSE)
                 )

j2.results <- data.frame(Model = c("Naive", "Simple Exponential Smoothing", "Holt Winters", "Regression"),
                 RMSE = c(j2.naive.RMSE, j2.se.RMSE, j2.holt.RMSE, j2.reg.RMSE)
                 )

j3.results <- data.frame(Model = c("Naive", "Simple Exponential Smoothing", "Holt Winters", "Regression"),
                 RMSE = c(j3.naive.RMSE, j3.se.RMSE, j3.holt.RMSE, j3.reg.RMSE)
                 )

j4.results <- data.frame(Model = c("Naive", "Simple Exponential Smoothing", "Holt Winters", "Regression"),
                 RMSE = c(j4.naive.RMSE, j4.se.RMSE, j4.holt.RMSE, j4.reg.RMSE)
                 )
...
j4.results
```
Junction 4 seems to be the most predictable, with the lowest RMSE values. No model significantly outperforms the Naive model for any junction. 
