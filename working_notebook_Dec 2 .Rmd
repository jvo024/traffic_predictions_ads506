...
## Junction 1
j1.split <- ts_split(ts.obj = j1.ts, sample.out = 30)
j1.train <- j1.split$train
j1.test <- j1.split$test

## Junction 2
j2.split <- ts_split(ts.obj = j2.ts, sample.out = 30)
...
j1.naive <- naive(j1.train, h = 30)
#summary(j1.naive)

j2.naive <- naive(j2.train, h = 30)
#summary(j2.naive)

# Calculate RMSE Values
j1.naive.RMSE <- sqrt(mean((((as.numeric(j1.naive$residuals))[-1])^2)))
j2.naive.RMSE <- sqrt(mean((((as.numeric(j2.naive$residuals))[-1])^2)))
...
j1.snaive <- snaive(j1.train, h = 30)
#summary(j1.naive)

j2.snaive <- snaive(j2.train, h = 30)
#summary(j2.naive)

# Calculate RMSE Values
j1.snaive.RMSE <- sqrt(mean((((as.numeric(j1.naive$residuals))[-1])^2)))
j2.snaive.RMSE <- sqrt(mean((((as.numeric(j2.naive$residuals))[-1])^2)))
...
j1.pred <- forecast(j1.sm, h = 30)
#summary(j1.se)

j2.sm <- ets(j2.train, model = "ZZZ")
j2.pred <- forecast(j2.sm, h = 30)
#summary(j2.se)

# Calculate RMSE Values
j1.sm.RMSE <- sqrt(mean((as.numeric(j1.sm$residuals))^2))
j2.sm.RMSE <- sqrt(mean((as.numeric(j2.sm$residuals))^2))
...
j1.holt <- holt(j1.train, h = 30)
#summary(j1.holt)

j2.holt <- holt(j2.train, h = 30)
#summary(j2.holt)

# Calculate RMSE Values
j1.holt.RMSE <- sqrt(mean((as.numeric(j1.holt$residuals))^2))
j2.holt.RMSE <- sqrt(mean((as.numeric(j2.holt$residuals))^2))
...
# Calculate RMSE Values
j1.reg.RMSE <- sqrt(mean((as.numeric(j1.regression$residuals))^2))
j2.reg.RMSE <- sqrt(mean((as.numeric(j2.regression$residuals))^2))
```
...
# Calculate RMSE Values
j1.autoarima.RMSE <- sqrt(mean((as.numeric(j1.autoarima$residuals))^2))
j2.autoarima.RMSE <- sqrt(mean((as.numeric(j2.autoarima$residuals))^2))
```
...
j1.train.external <- j1[1:578,] %>%
  select(-Date, -Junction)

j2.train.external <- j2[1:578,]%>%
  select(-Date, -Junction)
...
# Calculate RMSE Values
j1.autoarima.ex.RMSE <- sqrt(mean((as.numeric(j1.autoarima.ex$residuals))^2))
j2.autoarima.ex.RMSE <- sqrt(mean((as.numeric(j2.autoarima.ex$residuals))^2))
```
...
j1.results <- data.frame(
  Model = c("Naive", "Seasonal Naive", "Exponential Smoothing", "Holt Winters",
            "Regression", "Auto ARIMA", "Auto ARIMA with predictors "),
  RMSE.Junction1 = c(j1.naive.RMSE, j1.snaive.RMSE, j1.sm.RMSE, j1.holt.RMSE, j1.reg.RMSE,
           j1.autoarima.RMSE, j1.autoarima.ex.RMSE)
  )

j2.results <- data.frame(
  Model = c("Naive", "Seasonal Naive", "Exponential Smoothing", "Holt Winters",
            "Regression", "Auto ARIMA", "Auto ARIMA with predictors "),
  RMSE.Junction2 = c(j2.naive.RMSE, j2.snaive.RMSE, j2.sm.RMSE, j2.holt.RMSE, j2.reg.RMSE,
           j2.autoarima.RMSE, j2.autoarima.ex.RMSE)
  )
j1.results
j2.results
...
autoplot(j1.train) +
  autolayer(j1.pred, color = "green", series = "Smoothing", alpha = 0.5) +
...
# Plot best two models for Junction 1 
autoplot(j2.train) +
  autolayer(j2.pred, color = "green", series = "Smoothing", alpha = 0.5) +
